...
- `website-tutorials/index.html` — hand-crafted HTML landing page for the whole site. Not generated by Quarto; Quarto copies it into `_site/` during the build. **Do not suggest converting this to `index.qmd`.** The site started as `index.qmd` but was deliberately rewritten as raw HTML because Quarto imposed too many layout and styling limitations for a professional landing page. That decision is final.
- `website-tutorials/bio.md` — the editable source for the About section bio text. Edit this file when the bio needs updating; never edit the bio paragraphs directly in `index.html`.
- `website-tutorials/tutorials.qmd` — Quarto-rendered tutorials index page (linked from the navbar "All Tutorials" entry).
- `website-tutorials/_site/index.html` — the built output; open this in a browser to preview the landing page locally.
