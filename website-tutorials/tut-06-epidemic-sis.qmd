...
catching_valve_x  <- 5.5
catching_valve_y  <- 3.28
recovering_valve_x <- 5.5
recovering_valve_y <- 2.72

ggplot() +
  # ── Flows ──────────────────────────────────────────────────────────────────
  annotate("segment", x = 4.3, xend = 6.7,
           y = catching_valve_y, yend = catching_valve_y,
           linewidth = 2, colour = "#4e8cd4", arrow = flow_arrow) +
  annotate("segment", x = 6.7, xend = 4.3,
           y = recovering_valve_y, yend = recovering_valve_y,
           linewidth = 2, colour = "#d45b4e", arrow = flow_arrow) +

  # ── Info links: vertical chain (straight, stacked directly above catching) ─
  # prob. contact with sick → catching illness
  annotate("segment", x = 5.5, xend = 5.5, y = 4.88, yend = 3.90,
           colour = "grey40", linewidth = 0.5, arrow = info_arrow) +
  
  # ── Info links: curved ─────────────────────────────────────────────────────
  # Healthy People → total population
  geom_curve(
    data = data.frame(x = 2.3, y = 3.5, xend = 4.5, yend = 5.08),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = -0.25, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # Sick People → prob. contact with sick (direct: sick is the numerator)
  geom_curve(
    data = data.frame(x = 8.7, y = 3.5, xend = 6.5, yend = 5.08),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = 0.25, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # population interactions → catching valve
  geom_curve(
    data = data.frame(x = 1.9, y = 5.05, xend = 5.3, yend = 3.90),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = -0.35, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # prob. catching → catching valve
  geom_curve(
    data = data.frame(x = 9.1, y = 5.05, xend = 5.7, yend = 3.90),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = 0.35, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # duration of illness → recovering valve (straight, stacked directly below)
  annotate("segment", x = 5.5, xend = 5.5, y = 1.82, yend = 2.2,
           colour = "grey40", linewidth = 0.5, arrow = info_arrow) +
  ...
  # ── Valve circles (rendered after flow segments, before text) ──────────────
  annotate("point", x = catching_valve_x,   y = catching_valve_y,   size = 5, colour = "#4e8cd4") +
  annotate("point", x = recovering_valve_x, y = recovering_valve_y, size = 5, colour = "#d45b4e") +

  # ── Text (rendered last, on top of all geometry) ───────────────────────────
  # Stock labels
  ...
  annotate("text", x = 8.7, y = 3.0, label = "Sick\nPeople",
           size = 3.5, fontface = "bold") +
  # Flow labels
  annotate("text", x = catching_valve_x,   y = 3.65, label = "Catching\nIllness",
           size = 3.2, colour = "#4e8cd4") +
  annotate("text", x = recovering_valve_x, y = 2.38, label = "Recovering",
           size = 3.2, colour = "#d45b4e") +
  # Auxiliaries (bold — calculated values)
  annotate("text", x = 5.5, y = 5.2, label = "prob. contact\nwith sick",
           size = 3.0, fontface = "bold") +
  # Parameters (plain text)
  annotate("text", x = 1.5,  y = 5.05, label = "population\ninteractions", size = 3.0) +
  annotate("text", x = 9.5,  y = 5.05, label = "prob.\ncatching",          size = 3.0) +
  annotate("text", x = 5.5,  y = 1.6,  label = "duration of\nillness",     size = 3.0) +
  # Loop labels
  annotate("text", x = 1.5, y = 4.2, label = "B1  (−)", size = 3.8,
           colour = "#c0392b", fontface = "bold") +
  annotate("text", x = 9.5, y = 4.2, label = "R1  (+)", size = 3.8,
           colour = "#27ae60", fontface = "bold") +
  theme_void() +
  coord_cartesian(xlim = c(0, 11), ylim = c(0.3, 5.7)) +
...
model <- function(time, stocks, params) {
  with(as.list(c(stocks, params)), {
    ...
    a_prob_contact_sick <- s_sick / (s_healthy + s_sick)
