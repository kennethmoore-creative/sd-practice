...
ggplot() +
  ...
  geom_curve(
    data = data.frame(x = 2.15, y = 2.7, xend = 4.5, yend = 1.95),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = -0.2, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # Info: Desired Employees → Distance to Goal
  geom_curve(
    data = data.frame(x = 2.15, y = 1.12, xend = 4.2, yend = 1.72),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = 0.2, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # Info: Distance to Goal → Outflow valve
  geom_curve(
    data = data.frame(x = 5.3, y = 1.9, xend = valve_x, yend = valve_y - 0.12),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = 0.2, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # Info: Adjustment Time → Outflow valve
  geom_curve(
    data = data.frame(x = 7.5, y = 2.0, xend = valve_x + 0.2, yend = valve_y - 0.12),
    aes(x = x, y = y, xend = xend, yend = yend),
...
ggplot() +
  ...
  annotate("segment", x = 3.8, xend = 8.0, y = valve_y, yend = valve_y,
           linewidth = 2, colour = "#d45b4e", arrow = flow_arrow) +
  annotate("point", x = valve_x, y = valve_y, size = 5, colour = "#d45b4e") +
  annotate("text", x = valve_x, y = 3.65, label = "Flow",
           size = 3.2, colour = "#d45b4e") +
  # Sink cloud
  geom_polygon(data = make_cloud(8.7, valve_y), aes(x, y),
               fill = "white", colour = "black", linewidth = 0.7) +
  # Auxiliary: Adjustment Gap
  annotate("text", x = 4.8, y = 1.8, label = "Adjustment\nGap", size = 3.0, fontface = "bold") +
