...
ggplot() +
  ...
  # Info: deaths multiplier → deaths valve (B1 balancing loop)
  geom_curve(
  ...
  # Info: average lifetime → deaths valve
  geom_curve(
...
ggplot() +
  ...
  # Info: normalized nutrition → deaths valve (B1 balancing loop)
  geom_curve(
    data = data.frame(x = 6.3, y = 1.85, xend = deaths_valve_x, yend = deaths_valve_y - 0.12),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = 0.25, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # Info: stock → births valve (R1 reinforcing loop)
