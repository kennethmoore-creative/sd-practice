...
ggplot() +
  ...
  # Auxiliary chain (bold — calculated values)
  ...
  annotate("text", x = 10.1, y = 2.6,  label = "baseline average\nlifetime",  size = 3.0) +
  annotate("text", x = 2.9, y = 2.15, label = "area",               size = 3.0) +
  annotate("text", x = 2.9, y = 1.3,  label = "normal pop\ndensity", size = 3.0) +

  # Info: vertical chain (straight segments — elements stacked directly above each other)
  ...
  # Info: area → population density
  geom_curve(
    data = data.frame(x = 3.2, y = 2.15, xend = 4.7, yend = 2.65),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = -0.2, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # Info: normal pop density → normalized density
  geom_curve(
    data = data.frame(x = 3.2, y = 1.35, xend = 4.7, yend = 1.75),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = -0.2, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # Auxiliary: effective lifetime (bold — calculated value)
  annotate("text", x = 8.2, y = 1.5, label = "effective\nlifetime",
           size = 3.0, fontface = "bold") +
  # Info: deaths multiplier → effective lifetime
  geom_curve(
    data = data.frame(x = 6.3, y = 0.95, xend = 7.6, yend = 1.35),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = 0.2, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # Info: stock → births valve (R1 reinforcing loop)
  geom_curve(
    data = data.frame(x = 4.5, y = 3.45, xend = births_valve_x + 0.1, yend = births_valve_y - 0.12),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = -0.38, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # Info: stock → dying valve (B1 balancing loop)
  geom_curve(
    data = data.frame(x = 6.5, y = 3.45, xend = deaths_valve_x - 0.1, yend = deaths_valve_y - 0.12),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = 0.38, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  ...
  geom_curve(
    data = data.frame(x = 10.1, y = 2.42, xend = 8.6, yend = 1.5),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = -0.3, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  # Info: effective lifetime → dying valve
  geom_curve(
    data = data.frame(x = 8.5, xend = 8.55, y = 1.65, yend = deaths_valve_y - 0.12),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = 0.3, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
  ...
  annotate("text", x = 7.3, y = 2.1, label = "B1  (−)", size = 3.8,
...
params_s1 <- c(
  ...
  p_baseline_average_lifetime   = 4,     # years
  p_area               = 1,     # acres
...
model_s1 <- function(time, stocks, params) {
  with(as.list(c(stocks, params)), {
    ...
    c_population_density <- s_rabbit_population / p_area
    c_normalized_density <- c_population_density / p_normal_pop_density
    c_deaths_multiplier  <- deaths_mult_fn(c_normalized_density)
    c_effective_lifetime <- p_baseline_average_lifetime / c_deaths_multiplier
    ...
    return(list(c(ds_dt),
                birthing          = f_birthing,
                dying             = f_dying,
                normalized_density = c_normalized_density,
...
**Positive loop (R1) — Birthing:**
$$\text{rabbit population} \xrightarrow{+} \text{birthing} \xrightarrow{+} \text{rabbit population}$$

**Negative loop (B1) — Density-Dependent Dying:**
$$\text{rabbit population} \xrightarrow{+} \text{density} \xrightarrow{+} \text{deaths multiplier} \xrightarrow{-} \text{effective lifetime} \xrightarrow{-} \text{dying} \xrightarrow{-} \text{rabbit population}$$
...
$$\text{rabbit population} \xrightarrow{+} \text{food demand} \xrightarrow{-} \text{food per rabbit} \xrightarrow{-} \text{effective lifetime} \xrightarrow{-} \text{dying} \xrightarrow{-} \text{rabbit population}$$
...
ggplot() +
  ...
  # Info: vertical chain (straight — elements stacked directly above each other)
  annotate("segment", x = 5.5, xend = 5.5, y = 3.45, yend = 2.98,
           colour = "grey40", linewidth = 0.5, arrow = info_arrow) +
  annotate("segment", x = 5.5, xend = 5.5, y = 2.52, yend = 2.08,
           colour = "grey40", linewidth = 0.5, arrow = info_arrow) +
  ...
  geom_curve(
    data = data.frame(x = 3.2, y = 2.75, xend = 4.7, yend = 2.75),
    aes(x = x, y = y, xend = xend, yend = yend),
    curvature = -0.2, colour = "grey40", linewidth = 0.5, arrow = info_arrow
  ) +
...
params_s2 <- c(
  p_births_normal    = 1.5,    # 1/year
  p_baseline_average_lifetime = 4,      # years (baseline, when food is adequate)
...
Rather than a lookup function, deaths are driven by a modeled process and mechanistic equation:

$$c\_\text{effective lifetime} = \text{baseline average lifetime} \times \min(\text{normalised nutrition},\ 1)$$
$$f\_\text{dying} = \frac{\text{population}}{c\_\text{effective lifetime}}$$
...
Together they define the threshold at which food stress begins: `p_replenishment / p_rabbit_appetite = 153` is the population at which per-rabbit supply exactly meets appetite. Above 153, nutrition falls below 1 and the B1 loop tightens. The carrying capacity falls at `p_births_normal × p_baseline_average_lifetime × 153 = 6 × 153 ≈ 918` — the population where deaths balance births.
...
model_s2 <- function(time, stocks, params) {
  with(as.list(c(stocks, params)), {
    ...
    f_consuming            <- s_rabbit_population * p_rabbit_appetite
...
- **`nutritional_stress`** — the inverse of normalized nutrition, and the emergent analogue of Structure 1's `deaths_multiplier`. Both reach approximately **6** at equilibrium, for the same algebraic reason: birth rate × baseline average lifetime = 1.5 × 4 = 6. The lookup table and the food balance encode the same constraint — one empirically, one mechanistically.
...
$$\text{population} \to \text{density} \to \underbrace{\text{deaths multiplier} \to \text{effective lifetime}}_{\text{empirical lookup}} \to \text{dying} \to \text{population}$$

**Structure 2 — negative loop (B1, unpacked):**
$$\text{population} \to \underbrace{\text{food demand} \to \text{food per rabbit} \to \text{effective lifetime}}_{\text{mechanistic food balance}} \to \text{dying} \to \text{population}$$
